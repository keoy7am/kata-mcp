...
```bash
node scripts/observe-report.mjs            # 或 --project <dir>、--json
```

报表会把这份记录与 Claude Code 的 transcript 对起来——assistant 条目不带 prompt id，所以调用是**按 transcript 顺序归属**——然后打印：观测到的轮次中有多少调用了链、注入字节的中位数，以及每条链的「提供／被调用」次数。

**要看的是每条链那一列，不是总比率。** 总比率低不代表失败：多数轮次本来就不该套链，那正是 PASS 的用途。**不需要任何主观判断就有意义的数字是每条链的**——一条被提供上百次、从未被调用的链，说明它的触发语没有在起作用，而这是今天就能处理的。

它仍然无法告诉你的是：那一轮**是否应该**调用。那需要有人去读那些轮次并判断，这里没有任何东西能自动化它。
