...
- **这不是推荐。** 它是为一个人的环境做的，在那里感觉有用。没有任何东西显示这能移植；一个已经把同样纪律写在自己指令里的 harness，可能从它这里什么都拿不到。
...
「非琐碎」是规则不是模型：40 个字符以上（`KATA_GATE_MIN_CHARS`）且不是纯粹的应答（`ok`、`好`、`继续`……）。它刻意粗糙。模型自己「何时该路由」的判断正是受测物，所以不能同时当裁判。
