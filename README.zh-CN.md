...
- **对作者自己的 transcript 做前后对比，什么都没找到。** 约 4000 个交互轮次，按模型分层，排除构建这个工具的那些 session：一个模型的用户修正率降了（11.6% → 2.5%，n=119），另一个升了（6.3% → 11.4%，n=35），工具错误率没动，而且每一个「后」的轮次跑的 Claude Code 都比多数「前」的更新。这些是代理指标，不是质量——但里面没有任何可以拿来宣传的信号。脚本在 `scripts/before-after.py`，它读的是本机 Claude Code 的 transcript，任何人都能对自己的跑一次。
