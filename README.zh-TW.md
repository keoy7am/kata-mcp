...
報表會把這份記錄與 Claude Code 的 transcript 對起來——assistant 條目不帶 prompt id，所以呼叫是**依 transcript 順序歸屬**——然後印出：觀測到的輪次中有多少呼叫了鏈、注入位元組的中位數，以及每條鏈的「提供／被呼叫」次數。

**要看的是每條鏈那一欄，不是總比率。** 總比率低不代表失敗：多數輪次本來就不該套鏈，那正是 PASS 的用途。**不需要任何主觀判斷就有意義的數字是每條鏈的**——一條被提供上百次、從未被呼叫的鏈，代表它的觸發語沒有在運作，而這是今天就能處理的。

它仍然無法告訴你的是：那一輪**是否應該**呼叫。那需要有人去讀那些輪次並判斷，這裡沒有任何東西能自動化它。
